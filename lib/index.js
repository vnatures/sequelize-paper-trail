...
exports.init = (sequelize, optionsArg) => {
	...
	function getAssociations(instance) {
		...
		const modelAssociations = options.associations.filter(
			association => instance.constructor === association.model,
		);
		const previous = {};
		const current = {};
		...
		modelAssociations.forEach(modelAssociation => {
			...
			const prev = prevData && prevData.map(mapper);
			const dataValue = instance[fieldName];
			const cur = dataValue && dataValue.map(mapper);
	...
	function getDataVersions(previousVersion, currentVersion, instance) {
		...
		previousVersion = _.pick(previousVersion, allNonVirtualFields);
		previousVersion = _.omit(previousVersion, options.exclude);

		currentVersion = _.pick(currentVersion, allNonVirtualFields);
