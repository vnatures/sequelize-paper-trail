...
exports.init = (sequelize, optionsArg) => {
	...
	function createBeforeHook(operation) {
		const beforeHook = function beforeHook(instance, opt) {
			...
			if ('paperTrailAttributes' in instance) {
				previousVersion = _.pick(previousVersion, instance.paperTrailAttributes());
			} else {
			...
			if ('paperTrailAttributes' in instance) {
				currentVersion = _.pick(currentVersion, instance.paperTrailAttributes());
			} else {
